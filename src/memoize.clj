...
(defn memo-clear!
  ...
  ([f]
   (when-let [cache (cache-id f)]
     (swap! cache (constantly (cache/seed @cache {})))))
  ([f args]
   (when-let [cache (cache-id f)]
     (swap! cache (constantly (cache/evict @cache args))))))
...
(defn memo-reset!
  ...
  (when-let [cache (cache-id f)]
    (swap! cache
           (constantly (cache/seed @cache (derefable-seed base))))))
...
(defn memo-swap!
  ...
  world view, the assumption is that you will generally be calling it like:
        ...
        (def id (memo identity))
        (memo-swap! id cache/miss [13] :omg)
        (id 13)
        ;=> :omg

  You'll nearly always use `cache/miss` for this operation but
  ...
  in a `delay` so that items conform to `memoize`'s world view."
  ([f base]
   (when-let [cache (cache-id f)]
     (swap! cache
            (constantly (cache/seed @cache (derefable-seed base))))))
...
(defn memo
  "Used as a more flexible alternative to Clojure's core `memoization`
   ...
   `:memoize/args-fn` key, the value is assumed to be a
   ...
   You can access the memoization cache directly via the `:memoize/cache` key
...
(defn ^{:private true} !! [c]
  (println "WARNING - Deprecated construction method for"
           ...
           (str "(memoize/" c " function <base> <:" c "/threshold num>)")))
...
(defmacro ^{:private true} massert [condition msg]
  `(when-not ~condition
     (throw (new AssertionError (str "memoize/" ~msg "\n" (pr-str '~condition))))))
...
(def-deprecated fifo
  "DEPRECATED: Please use memoize/fifo instead."
  ([f] (memo-fifo f 32 {}))
...
(def-deprecated lru
  "DEPRECATED: Please use memoize/lru instead."
  ([f] (memo-lru f 32))
...
(def-deprecated ttl
  "DEPRECATED: Please use memoize/ttl instead."
  ([f] (memo-ttl f 3000 {}))
...
(def-deprecated lu
  "DEPRECATED: Please use memoize/lu instead."
  ([f] (memo-lu f 32))
