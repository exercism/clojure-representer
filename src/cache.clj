...
(defn through
  ...
  ([wrap-fn value-fn cache item]
   (if (cache/has? cache item)
     (cache/hit cache item)
     (cache/miss cache item (wrap-fn #(value-fn %) item)))))
...
(defn fifo-cache-factory
  ...
  (cache/seed (FIFOCache. {} clojure.lang.PersistentQueue/EMPTY threshold) base))
...
(defn lru-cache-factory
  ...
  {:pre [(number? threshold) (< 0 threshold)
         (map? base)]}
  (cache/seed (LRUCache. {} (priority-map/priority-map) 0 threshold) base))
...
(defn ttl-cache-factory
  ...
  {:pre [(number? ttl) (<= 0 ttl)
         (map? base)]}
  (cache/seed (TTLCacheQ. {} {} clojure.lang.PersistentQueue/EMPTY 0 ttl) base))
...
(defn lu-cache-factory
  ...
  {:pre [(number? threshold) (< 0 threshold)
         (map? base)]}
  (cache/seed (LUCache. {} (priority-map/priority-map) threshold) base))
...
(defn lirs-cache-factory
  ...
  {:pre [(number? s-history-limit) (< 0 s-history-limit)
         ...
         (map? base)]}
  (cache/seed (LIRSCache. {} {} {} 0 s-history-limit q-history-limit) base))
...
(defn soft-cache-factory
  ...
  [base]
  {:pre [(map? base)]}
  (cache/seed (SoftCache. (ConcurrentHashMap.) (ConcurrentHashMap.) (ReferenceQueue.))
                           base))